...
-(void)drawRect:(CGRect)rect
{
    ...
    //方向设置
    _windDirection = [[UILabel alloc]initWithFrame:CGRectMake(self.bounds.size.width - self.bounds.size.width/4.5, self.bounds.size.height/4.5, 100, 40)];
...
- (NSString *)sendtitle:(NSString *)gettitle
{
    ...
    if ([gettitle isEqualToString:@"A区"]) {
        //1.请求地址
        NSURL *url = [NSURL URLWithString:urlZoneA72];
        //2.创建请求
        ...
        NSURL *url = [NSURL URLWithString:urlZoneB72];
        //2.创建请求
        NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
        //3.连接服务器
        NSURLConnection *connection = [[NSURLConnection alloc]initWithRequest:request delegate:self];
    }else if ([gettitle isEqualToString:@"C区"]){
        NSURL *url = [NSURL URLWithString:urlZoneC72];
        //2.创建请求
        NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
        //3.连接服务器
        NSURLConnection *connection = [[NSURLConnection alloc]initWithRequest:request delegate:self];
    }else if ([gettitle isEqualToString:@"D区"]){
        NSURL *url = [NSURL URLWithString:urlZoneD72];
        //2.创建请求
        NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
        //3.连接服务器
        NSURLConnection *connection = [[NSURLConnection alloc]initWithRequest:request delegate:self];
