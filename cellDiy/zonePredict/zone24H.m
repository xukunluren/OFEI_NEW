...
//    _wind.font = [UIFont fontWithName:@"Arial-BoldItalicMT" size:26];
    ...
    _windDirection.font = [UIFont fontWithName:@"TimesNewRomanPS-ItalicMT" size:24];
...
//    _windAvg.font = [UIFont fontWithName:@"AmericanTypewriter-Bold" size:26];
    ...
    _detailWind.font = [UIFont fontWithName:@"Arial" size:14];
    ...
    _detailWave.font = [UIFont fontWithName:@"Arial" size:14];
...
//    NSString *maxlevel = [self level:maxwind];
    ...
    _detailWind.text = [NSString stringWithFormat:@"风力等级:%@级，风速平均值:%@m/s",minlevel,avgwind];
}
...
- (NSString *)sendtitle:(NSString *)gettitle
{
    ...
    if ([gettitle isEqualToString:@"A区"]) {
        ...
        NSURL *url = [NSURL URLWithString:urlZoneB24];
        //2.创建请求
        NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
        //3.连接服务器
        NSURLConnection *connection = [[NSURLConnection alloc]initWithRequest:request delegate:self];
    }else if ([gettitle isEqualToString:@"C区"]){
        NSURL *url = [NSURL URLWithString:urlZoneC24];
        //2.创建请求
        NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
        //3.连接服务器
        NSURLConnection *connection = [[NSURLConnection alloc]initWithRequest:request delegate:self];
    }else if ([gettitle isEqualToString:@"D区"]){
        NSURL *url = [NSURL URLWithString:urlZoneD24];
        //2.创建请求
        NSURLRequest *request = [[NSURLRequest alloc]initWithURL:url cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10];
        //3.连接服务器
        NSURLConnection *connection = [[NSURLConnection alloc]initWithRequest:request delegate:self];
